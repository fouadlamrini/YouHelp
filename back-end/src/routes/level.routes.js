...
const { createLevelSchema, updateLevelSchema } = require("../validators/level.validator");
...
router.post("/", auth, requireRole(["super_admin"]), validate(createLevelSchema), controller.create);
router.put("/:id", auth, requireRole(["super_admin"]), validate(updateLevelSchema), controller.update);
